...
#ifndef LIMOI_VIDEOSRC_COMPONENT_H__
#define LIMOI_VIDEOSRC_COMPONENT_H__
...
typedef struct
{
     ...
     */
    int (*read) (OMX_HANDLETYPE hComp, OMX_BUFFERHEADERTYPE * out);
                   

} limoi_videosrc_callbacks_t;

/** limoi_video_src component private data structure. */
struct limoi_videosrc_priv_t
{
    limoi_videosrc_callbacks_t callbacks; /** src callbacks. */
};


/** opaque limoi video src private data structure. */
typedef struct limoi_videosrc_priv_t limoi_videosrc_priv_t;

/** limoi video src component base class. */
typedef struct
{
    limoi_component_t parent;
    limoi_videosrc_priv_t * priv; /** opaque private data. */
} limoi_videosrc_t;

#define LIMOI_VIDEOSRC(c) ((limoi_videosrc_t *)OMX_COMPONENT(c))
 ...
 *        
 */
OMX_ERRORTYPE limoi_videosrc_deinit (OMX_HANDLETYPE hComp);
 ...
 */
OMX_ERRORTYPE limoi_videosrc_init (
 ...
 *        input video coding type/format.
 */
OMX_ERRORTYPE limoi_videosrc_init_sync (
    OMX_HANDLETYPE * hComp,
	int bytes,
	const char * comp_name,
	const char * comp_version,
 ...
 */
int limoi_videosrc_set_callbacks (
    OMX_HANDLETYPE hComp,
    limoi_videosrc_callbacks_t * callbacks);
