...
#ifndef LIMOI_VIDEOIVA01_COMPONENT_H__
#define LIMOI_VIDEOIVA01_COMPONENT_H__
...
typedef struct
{
    ...
    int (*main) (OMX_HANDLETYPE hComp, 
			    ...
			    void **data);                 
} limoi_videoIVA01_callbacks_t;

/** limoi_video_src component private data structure. */
struct limoi_videoIVA01_priv_t
{
    limoi_videoIVA01_callbacks_t callbacks; /** src callbacks. */
};


/** opaque limoi video src private data structure. */
typedef struct limoi_videoIVA01_priv_t limoi_videoIVA01_priv_t;

/** limoi video src component base class. */
typedef struct
{
    limoi_component_t parent;
    limoi_videoIVA01_priv_t * priv; /** opaque private data. */
} limoi_videoIVA01_t;

#define LIMOI_VIDEOIVA01(c) ((limoi_videoIVA01_t *)OMX_COMPONENT(c))
 ...
 *        
 */
OMX_ERRORTYPE limoi_videoIVA01_deinit (OMX_HANDLETYPE hComp);
 ...
 */
OMX_ERRORTYPE limoi_videoIVA01_init (
 ...
 *        input video coding type/format.
 */
OMX_ERRORTYPE limoi_videoIVA01_init_sync (
    OMX_HANDLETYPE * hComp,
	int bytes,
	const char * comp_name,
	const char * comp_version,
 ...
 */
int limoi_videoIVA01_set_callbacks (
    OMX_HANDLETYPE hComp,
    limoi_videoIVA01_callbacks_t * callbacks);
