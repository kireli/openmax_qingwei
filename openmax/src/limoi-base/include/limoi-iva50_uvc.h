...
#ifndef LIMOI_IVA50_UVC_COMPONENT_H__
#define LIMOI_IVA50_UVC_COMPONENT_H__

#include "limoi-component.h"
#include "component.h"

typedef enum{
	IVA50_UVC_SPK_IR_DEPTH_MODE,
    IVA50_UVC_RGB_IR_SPK_MODE,
	IVA50_UVC_IR_DEPTH_MODE,
    IVA50_UVC_DUAL_IR_MODE,
	IVA50_UVC_RGB_MODE,
	IVA50_UVC_CAT_EYE_MODE,
	IVA50_UVC_CAP_MODE,
	IVA50_UVC_NONE_MODE,
}iva50_uvc_mode_e;
...
typedef struct
{
    ...
    int (*main) (OMX_HANDLETYPE hComp, OMX_BUFFERHEADERTYPE * in, 
									    ...
									    OMX_BUFFERHEADERTYPE * in4,
									    int port);
                   

} limoi_iva50_uvc_callbacks_t;

/** limoi_video_src component private data structure. */
struct limoi_iva50_uvc_priv_t
{
    limoi_iva50_uvc_callbacks_t callbacks; /** src callbacks. */
};


/** opaque limoi video src private data structure. */
typedef struct limoi_iva50_uvc_priv_t limoi_iva50_uvc_priv_t;

/** limoi video src component base class. */
typedef struct
{
    limoi_component_t parent;
    limoi_iva50_uvc_priv_t * priv; /** opaque private data. */
} limoi_iva50_uvc_t;

#define LIMOI_IVA50_UVC(c) ((limoi_iva50_uvc_t *)OMX_COMPONENT(c))
 ...
 *        
 */
OMX_ERRORTYPE limoi_iva50_uvc_deinit (OMX_HANDLETYPE hComp);
 ...
 */
OMX_ERRORTYPE limoi_iva50_uvc_init (
 ...
 *        input video coding type/format.
 */
OMX_ERRORTYPE limoi_iva50_uvc_init_sync (
    OMX_HANDLETYPE * hComp,
	int bytes,
	const char * comp_name,
	const char * comp_version,
 ...
 */
int limoi_iva50_uvc_set_callbacks (
    OMX_HANDLETYPE hComp,
    limoi_iva50_uvc_callbacks_t *callbacks);
